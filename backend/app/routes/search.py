...
def generate_snippet(text: str, query: str) -> str:
    ...
    final_sentences = []
    for score, sent, matched in top_sents:
        highlighted = sent
...
def search_documents_metadata(
    ...
    from app.models.search import SearchLog, CrawledPage
    import time
    ...
    for key in all_keys:
        ...
        if facets.get("type"):
            f_type = facets["type"]
            if f_type == "WEB" and obj_type != "web": continue
            if f_type != "WEB" and obj_type == "web": continue
        ...
        if obj_type == "file" and "confidence" in facets:
            ...
            if op == ">" and not (score_val > val): continue
            if op == "<" and not (score_val < val): continue
            if op == ">=" and not (score_val >= val): continue
            if op == "<=" and not (score_val <= val): continue
            if op == "=" and not (score_val == val): continue
